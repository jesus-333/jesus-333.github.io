...
- `cv-mirror.yml`: mirrors `cv_tex/` into the standalone `cv_claude` repo (for Overleaf) when the LaTeX sources change
- `cv-sync.yml`: manual trigger that regenerates the derived CV files from `common_material/cv.toml`
