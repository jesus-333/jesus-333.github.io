...
- [Customize](#customize)
  ...
  - [Modifying the CV information](#modifying-the-cv-information)
    - [RenderCV format (what this site renders)](#rendercv-format-what-this-site-renders)
    - [JSONResume format](#jsonresume-format)
    - [Switching between them](#switching-between-them)
    - [The PDF download button](#the-pdf-download-button)
...
> **This site does not edit CV data by hand.** Everything on `/cv/` is generated from
> [`common_material/cv.toml`](../common_material/cv.toml), the single source of truth shared with
> the LaTeX CV in [`cv_tex/`](../cv_tex/). Edit the TOML and run `make cv`; see
> [`common_material/README.md`](../common_material/README.md) for the full picture.
>
> The generated files — [`_data/cv.yml`](../_data/cv.yml) and
> [`assets/json/resume.json`](../assets/json/resume.json) — carry a "do not edit" banner, and
> `unit-tests.yml` fails any push where they have drifted from the TOML.
>
> The upstream template's RenderCV pipeline (`assets/rendercv/` and
> `.github/workflows/render-cv.yml`) has been removed: it rendered a second, competing PDF
> through Typst and auto-committed it on every push. The LaTeX project in `cv_tex/` is the only
> PDF generator now, and `make cv-pdf` publishes its output to `assets/pdf/`.

The two data formats below still exist as _outputs_, and `cv_format` still selects between them.

### RenderCV format (what this site renders)

[`_data/cv.yml`](../_data/cv.yml) uses the [RenderCV](https://rendercv.com/) YAML format. It is
what [`_pages/cv.md`](../_pages/cv.md) renders, because that page sets `cv_format: rendercv`.
Generated by `bin/cv/build_cv_yml.py`.

### JSONResume format

[`assets/json/resume.json`](../assets/json/resume.json) uses the
[JSONResume](https://jsonresume.org/) standard. It is generated in parallel by
`bin/cv/build_resume_json.py` so the two can never disagree, and is served but not rendered
unless you switch formats.
...
Two files carry a `cv_pdf` key and both must agree: [`_pages/cv.md`](../_pages/cv.md) (the button
on `/cv/`) and [`_data/socials.yml`](../_data/socials.yml) (the CV icon in the social row). They
are hand-owned; `bin/cv/build_pdf.sh` warns when they do not point at the PDF it just built,
rather than rewriting front matter it does not own.
